...
### Scenes and Situations
 ...
 - [ ]  Athandra (nearby town)
	Has only basic services, such as a small shop, and is known for a special store called the Everlight Emporium. The shopkeeper, a human man named Erasmus Everlight, is a different age every time the players enter the shop, but Erasmus himself doesn't seem to notice. He carries with him, and can be convinced to sell, an ancient lantern that he claims can be used to seek out magical creatures ([[Lantern of Seeking]], with a yellow gem already in place so it can be used to find sphinxes, costing 250gp).
